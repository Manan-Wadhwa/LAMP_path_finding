...
OUT_GPKG = r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\ElBagawat_Master.gpkg"
OUT_CSV = r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\ElBagawat_Crosswalk.csv"
OUT_3D = r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\3D_Blueprint.csv"
...
if __name__ == "__main__":
    ...
    walls_gdf.to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\walls.geojson", driver="GeoJSON")
    
    # Save the 4 door versions
    if len(og_doors) > 0: gpd.GeoDataFrame(og_doors, geometry=[d['geom'] for d in og_doors], crs=footprints.crs).to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\doors_og.geojson", driver="GeoJSON")
    gpd.GeoDataFrame(greedy_doors, geometry=[d['geom'] for d in greedy_doors], crs=footprints.crs).to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\doors_greedy.geojson", driver="GeoJSON")
    gpd.GeoDataFrame(collinear_doors, geometry=[d['geom'] for d in collinear_doors], crs=footprints.crs).to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\doors_collinear.geojson", driver="GeoJSON")
    gpd.GeoDataFrame(boolean_doors, geometry=[d['geom'] for d in boolean_doors], crs=footprints.crs).to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\doors_boolean.geojson", driver="GeoJSON")
    
    if len(native_doors) > 0: gpd.GeoDataFrame(native_doors, geometry=[d['geom'] for d in native_doors], crs=footprints.crs).to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\doors_native.geojson", driver="GeoJSON")
    if len(local_doors) > 0: gpd.GeoDataFrame(local_doors, geometry=[d['geom'] for d in local_doors], crs=footprints.crs).to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\doors_local.geojson", driver="GeoJSON")
    if len(hybrid_doors) > 0: gpd.GeoDataFrame(hybrid_doors, geometry=[d['geom'] for d in hybrid_doors], crs=footprints.crs).to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\doors_hybrid.geojson", driver="GeoJSON")
    
    # And buildings for the hover effect
    footprints.to_crs("EPSG:4326").to_file(r"C:\Users\Public\LAMP_DataStore\ElBagawat\200_Projects\210_GSOC\code-manan\outputs\buildings.geojson", driver="GeoJSON")
